...
switch ( $source ) {
	...
	case 'featured_image':
		...
		if ( strpos( $image, 'unsplash') !== false ) {
			...
			$img_url = $image;
			if ( $img_size != '' ) $img_url .= $img_size;

			$img = array(
				'thumbnail' => '<img class="vc_single_image-img" src="' . esc_url( $img_url ) . '" />'
			);

		} else if ( strpos( $image, 'http') !== false ) {

			$img_url = $image;
			if ( $img_size != '' && strpos( $image, 'media.veented') !== false && $img_size != 'full' && $img_size != 'thumbnail' ) {
