    ...
    function PluginManager(){
        ...
        function ajax_callback( response ) {
        	...
        	if ( typeof response == 'object' && typeof response.message != 'undefined' ) {
        		...
        		var body = response.replace(/^[\s\S]*<body.*?>|<\/body>[\s\S]*$/ig, '');
        		var $body = $(body);
        		if ( $body.find( '.vc-page-logo' ).length > 0 ) { 
        			// Fix if plugins redirect after activation to their own subpage
