				...
				public function theme_setup_wizard_content($content){
					if($this->is_possible_upgrade()){
						array_unshift_assoc($content,'upgrade',array(
							'title' => esc_html__( 'Upgrade', 'engage' ),
							'description' => esc_html__( 'Upgrade Content and Settings', 'engage' ),
							'pending' => esc_html__( 'Pending.', 'engage' ),
							'installing' => esc_html__( 'Installing Updates.', 'engage' ),
							'success' => esc_html__( 'Success.', 'engage' ),
							'checked' => 1
						));
					}
					return $content;
