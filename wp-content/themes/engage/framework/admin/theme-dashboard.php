...
if ( !class_exists( 'Engage_Dashboard' ) ) {
	...
	class Engage_Dashboard {
		    ...
		    <div class="engage-grid-container engage-grid-bg">
		    	<div class="engage-grid-row">
			    	<div class="engage-grid-col col-12">
			    		...
			    		<p><?php esc_html_e( 'In order to access all theme features, please install and activate the "Engage Core" plugin.', 'engage' ); ?></p>
			    		<a href="<?php echo esc_url( admin_url( 'admin.php?page=engage-plugins' ) ); ?>" class="engage-more-link"><?php esc_html_e( 'Activate the plugin now', 'engage' ); ?><i class="fa fa-angle-right"></i></a>
			    	</div>
		    	</div>
                ...
                <div class="engage-grid-row">
                    <div class="engage-grid-col col-6">
                        ...
                        if ( isset( $_POST[ $name_api_key ] ) ) {
                            ...
                            $value_api_key = sanitize_text_field( $_POST[ $name_api_key ] );

                            if ( engage_verify_envato_api_token( $_POST[ $name_api_key ] ) == true ) {

                                if ( !get_option( $name_api_key ) && get_option( $name_api_key ) != '' ) {
                                    add_option( $name_api_key, sanitize_text_field( $value_api_key ) );
                                } else {
                                    update_option( $name_api_key, sanitize_text_field( $value_api_key ) );
                                }
	    ...
	    ?>
	    	...
	    	<p class="engage-page-desc"><?php echo esc_html__( "This is a place where you can easily manage all Engage theme related plugins. The theme requires only two plugins: 'Engage Core' and 'WPBakery Page Builder' (former Visual Composer). The other ones aren't mandatory.", 'engage' ); ?></p>
	        ...
	        $plugins = array(
	        	...
	        	'the-events-calendar' => array(
	        		...
	        		'name' => 'Classic Editor',
	        		'slug' => 'classic-editor',
							'desc' => esc_html__( 'Enables the previous Classic Editor (Pre-Gutenberg) and the old-style Edit Post screen with TinyMCE, Meta Boxes, etc.', 'engage' )
	        	),
	        );
        				...
        				$item_classes = 'plugin-active';
	        				...
	        				if ( ! $tgmpa_instance->is_plugin_installed( $plugin['slug'] ) ) {
	        					...
	        					$item_classes = 'plugin-inactive';
									} elseif ( ! call_user_func( array( $tgmpa_instance, 'is_plugin_active' ), $plugin['slug'] ) ) {
	        					$button_label = esc_html__( 'Activate', 'engage' );
	        				...
	        				echo '<img src="' . esc_url( $img_path . $plugin_id . '.png' ) . '">';

	        				echo '</div>';

	        				echo '<div class="theme-id-container"><h2 class="theme-name" id="engage-demo-name">';
	        ...
	        <div class="engage-grid-container engage-grid-bg">
	        	<div class="engage-grid-row">
	    	    	...
	    	    	<div class="engage-grid-col col-4">
	    	    		...
	    	    		<p><?php esc_html_e( 'Have any other questions? Just submit a support ticket and we will be more than glad to help!', 'engage' ); ?></p>
