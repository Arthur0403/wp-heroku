    ...
    if ( ! class_exists( 'ReduxFramework_extension_metaboxes' ) ) {
        ...
        class ReduxFramework_extension_metaboxes {
            ...
            public function __construct( $parent ) {
                ...
                $file_name = 'metaboxes';
                $this->_extension_url = ENGAGE_CORE_URI . '/theme-panel/extensions/' . $file_name . '/';
                        ...
                        foreach ( $box['post_types'] as $posttype ) {
                            if ( isset( $box['title'] ) ) {
                                ...
                                if ( isset( $box['sections'] ) && count( $box['sections'] ) == 1 && isset( $box['sections'][0]['fields'] ) && count( $box['sections'][0]['fields'] ) == 1 && isset( $box['sections'][0]['fields'][0]['title'] ) ) {
                                    ...
                                    $title = ucfirst( $posttype ) . " " . esc_html__( 'Options', 'engage' );
                                }
...
?>
                <div class="redux-container<?php if ( $sidebar ) { echo ' redux-has-sections'; } else { echo ' redux-no-sections'; } ?> redux-box-<?php echo '' . $metabox['args']['position']; ?>">
                    <div class="redux-notices">
                        <?php if ( $sidebar ) { ?>
                            <div class="saved_notice admin-notice notice-blue" style="display:none;">
                                <strong><?php echo apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'engage' ) ) ?></strong>
                            </div>
                            <div class="redux-save-warn notice-yellow">
                                <strong><?php echo apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'engage' ) ) ?></strong>
                            </div>
                        <?php } ?>
                        <div class="redux-field-errors notice-red">
                            <strong>
                                <span></span> <?php echo esc_html__( 'error(s) were found!', 'engage' ) ?>
                            </strong>
                        </div>
                        <div class="redux-field-warnings notice-yellow">
                            <strong>
                                <span></span> <?php echo esc_html__( 'warning(s) were found!', 'engage' ) ?>
                            </strong>
                        </div>
                    </div>
<?php
                    echo '<a href="javascript:void(0);" class="expand_options hide" style="display:none;">' . esc_html__( 'Expand', 'engage' ) . '</a>';
                    if ( $sidebar ) {
?>
...
<?php
                                foreach ( $sections as $sKey => $section ) {
                                	...
                                	if( array_key_exists( 'post_type' , $section ) && !in_array( get_post_type( get_the_ID() ), $section['post_type'] ) ) {
                                		// Veented
                                	} else {
...
<?php
            ...
            function meta_boxes_show_errors() {
                if ( isset( $this->notices['errors'] ) && ! empty( $this->notices['errors'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['errors'] ) . ' ' . esc_html__( 'error(s) were found!', 'engage' ) . '</strong></p>';
                    echo '</div>';
                }
                ...
                if ( isset( $this->notices['warnings'] ) && ! empty( $this->notices['warnings'] ) ) {
                    ...
                    echo '<p><strong><span></span> ' . count( $this->notices['warnings'] ) . ' ' . esc_html__( 'warnings(s) were found!', 'engage' ) . '</strong></p>';
                    echo '</div>';
                }
