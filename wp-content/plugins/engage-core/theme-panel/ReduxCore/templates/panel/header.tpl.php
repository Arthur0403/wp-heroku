<?php
    ...
    $tip_title = esc_html__( 'Developer Mode Enabled', 'engage' );
    ...
    if ( $this->parent->dev_mode_forced ) {
        ...
        if ( Redux_Helpers::isWpDebug() ) {
            $is_debug  = true;
            $debug_bit = esc_html__( 'WP_DEBUG is enabled', 'engage' );
        }

        $localhost_bit = '';
        if ( Redux_Helpers::isLocalHost() ) {
            $is_localhost  = true;
            $localhost_bit = esc_html__( 'you are working in a localhost environment', 'engage' );
        }

        $conjunction_bit = '';
        if ( $is_localhost && $is_debug ) {
            $conjunction_bit = ' ' . esc_html__( 'and', 'engage' ) . ' ';
        }

        $tip_msg = esc_html__( 'This has been automatically enabled because', 'engage' ) . ' ' . $debug_bit . $conjunction_bit . $localhost_bit . '.';
    } else {
        $tip_msg = esc_html__( 'If you are not a developer, your theme/plugin author shipped with developer mode enabled. Contact them directly to fix it.', 'engage' );
    }
        ...
        <div class="display_header">
            ...
            <?php if ( isset( $this->parent->args['dev_mode'] ) && $this->parent->args['dev_mode'] ) { ?>
                <div class="redux-dev-mode-notice-container redux-dev-qtip"
                    ...
                    <span
                        class="redux-dev-mode-notice"><?php esc_html_e( 'Developer Mode Enabled', 'engage' ); ?></span>
                </div>
            <?php } elseif (isset($this->parent->args['forced_dev_mode_off']) && $this->parent->args['forced_dev_mode_off'] == true ) { ?>
                ...
                <div class="redux-dev-mode-notice-container redux-dev-qtip" 
                     ...
                     qtip-content="<?php echo esc_attr( $tip_msg ); ?>">
                    <span
                        class="redux-dev-mode-notice" style="background-color: #FF001D;"><?php esc_html_e( 'FORCED DEV MODE OFF ENABLED', 'engage' ); ?></span>
                </div>
            
            <?php } ?>

            <!--<h2><?php echo wp_kses_post( $this->parent->args['display_name'] ); ?></h2>-->
            <img src="<?php echo get_template_directory_uri() . '/img/logos/logo-light.png'; ?>" alt>
