        ...
        class ReduxFramework {
                ...
                if ( strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_stylesheet_directory() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( get_template_directory_uri() ) ) !== false || strpos( Redux_Helpers::cleanFilePath( __FILE__ ), Redux_Helpers::cleanFilePath( WP_CONTENT_DIR . '/themes/' ) ) !== false ) {
                    ...
                    if ( strpos( self::$_dir, ABSPATH ) === false ) {
                        ...
                        foreach ( get_plugins() as $key => $value ) {
                            if ( call_user_func( 'is_plugin_active', $key ) && strpos( $key, 'redux-framework.php' ) !== false ) {
            ...
            public function __construct( $sections = array(), $args = array(), $extra_tabs = array() ) {
                ...
                if ( empty ( $this->args['footer_credit'] ) ) {
                    $this->args['footer_credit'] = '<span id="footer-thankyou">' . sprintf( esc_html__( 'Options panel created using %1$s', 'engage' ), '<a href="' . esc_url( $this->framework_url ) . '" target="_blank">' . esc_html__( 'Redux Framework', 'engage' ) . '</a> v' . self::$_version ) . '</span>';
                }

                if ( empty ( $this->args['menu_title'] ) ) {
                    $this->args['menu_title'] = esc_html__( 'Options', 'engage' );
                }

                if ( empty ( $this->args['page_title'] ) ) {
                    $this->args['page_title'] = esc_html__( 'Options', 'engage' );
                }
                ...
                if ( ! empty ( $this->args['opt_name'] ) ) {
                    ...
                    // Admin Bar menu
                    add_action( 'admin_' . 'bar_menu', array(
                        $this,
                        '_admin_' . 'bar_menu'
                    ...
                    if ( $this->args['database'] == "network" && $this->args['network_admin'] ) {
                        ...
                        add_action( 'admin_' . 'bar_menu', array( $this, 'network_admin_bar' ), 999 );
                    }
                    // Ajax saving!!!
            ...
            private function _internationalization() {
                 ...
                 * @param string     'engage'  text domain
                 */
                //                $locale = apply_filters( "redux/textdomain/{$this->args['opt_name']}", get_locale(), 'engage' );
                //
                ...
                $loaded = load_plugin_textdomain( 'engage', false, $basepath . 'languages');

                if ( !$loaded ){
                    $loaded = load_muplugin_textdomain( 'engage', $basepath . 'languages' );
                }

                if ( !$loaded ){
                    $loaded = load_theme_textdomain( 'engage', $basepath . 'languages' );
                }

                if ( ! $loaded ) {
                    $locale = apply_filters( 'plugin_locale', get_locale(), 'engage' );
                    $mofile = dirname( __FILE__ ) . '/languages/redux-framework-' . $locale . '.mo';
                    load_textdomain( 'engage', $mofile );
                }
            }
...
//                    }
            ...
            public function _load_page() {
                ...
                if ( true == $this->show_hints ) {
                    ...
                    $dismiss = 'true';
                    $s       = esc_html__( 'Enable', 'engage' );

                    // Values for disabling hints.
                    if ( 'true' == $hint_status ) {
                        $dismiss = 'false';
                        $s       = esc_html__( 'Disable', 'engage' );
                    }

                    // Make URL
                    $url = '<a class="redux_hint_status" href="?dismiss=' . $dismiss . '&amp;id=hints&amp;page=' . $curPage . '&amp;tab=' . $curTab . '">' . $s . ' hints</a>';

                    $event = esc_html__( 'moving the mouse over', 'engage' );
                    if ( 'click' == $this->args['hints']['tip_effect']['show']['event'] ) {
                        $event = esc_html__( 'clicking', 'engage' );
                    }

                    // Construct message
                    $msg = sprintf( esc_html__( 'Hints are tooltips that popup when %d the hint icon, offering addition information about the field in which they appear.  They can be %d d by using the link below.', 'engage' ), $event, strtolower( $s ) ) . '<br/><br/>' . $url;

                    // Construct hint tab
                    $tab = array(
                        'id'      => 'redux-hint-tab',
                        'title'   => esc_html__( 'Hints', 'engage' ),
                        'content' => '<p>' . $msg . '</p>'
                    );
            ...
            private function get_default_output_string( $field ) {
                ...
                if ( ! empty ( $default_output ) ) {
                    $default_output = esc_html__( 'Default', 'engage' ) . ": " . substr( $default_output, 0, - 2 );
                }
            ...
            public function ajax_save() {
                if ( ! wp_verify_nonce( $_REQUEST['nonce'], "redux_ajax_nonce" . $this->args['opt_name'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid security credential.  Please reload the page and try again.', 'engage' ),
                        'action' => ''
                    ) );
                ...
                if ( ! self::current_user_can( $this->args['page_permissions'] ) ) {
                    echo json_encode( array(
                        'status' => esc_html__( 'Invalid user capability.  Please reload the page and try again.', 'engage' ),
                        'action' => ''
                    ) );
                ...
                if ( ! empty ( $_POST['data'] ) && ! empty ( $redux->args['opt_name'] ) ) {
                    ...
                    if ( $do_reload || ( isset ( $values['defaults'] ) && ! empty ( $values['defaults'] ) ) || ( isset ( $values['defaults-section'] ) && ! empty ( $values['defaults-section'] ) ) || ( isset ( $values['import_code'] ) && ! empty ($values['import_code']) ) || ( isset ( $values['import_link'] ) && ! empty ($values['import_link']) ) ) {
                        ...
                        echo json_encode( array( 'status' => esc_html__( 'Your panel has no fields. Nothing to save.', 'engage' ) ) );
                    }
                }
                if ( isset ( $this->transients['run_compiler'] ) && $this->transients['run_compiler'] ) {
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                        $msg = esc_html__( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>admin_bar_links</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'engage' );
                            ...
                            if ( is_array( $arr ) && ! empty( $arr ) ) {
                                foreach ( $arr as $x => $y ) {
                                    if (!$this->omit_share_icons) {
                                        if ( strpos( strtolower( $y ), 'redux' ) !== false ) {
                                            $msg = esc_html__( '<strong>Redux Framework Notice: </strong>There are references to the Redux Framework support site in your config\'s <code>share_icons</code> argument.  This is sample data.  Please change or remove this data before shipping your product.', 'engage' );
