    ...
    if ( ! class_exists( 'reduxCorePanel' ) ) {
        ...
        class reduxCorePanel {
            ...
            private function panel_template() {
                ...
                // Do we support JS?
                echo '<noscript><div class="no-js">' . esc_html__( 'Warning- This options panel will not work properly without javascript!', 'engage' ) . '</div></noscript>';
            ...
            function notification_bar() {
                ...
                if ( isset( $this->parent->transients['last_save_mode'] ) ) {
                    ...
                    if ( $this->parent->transients['last_save_mode'] == "import" ) {
                         ...
                         */
                        echo '<div class="admin-notice notice-blue saved_notice"><strong>' . apply_filters( "redux-imported-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings Imported!', 'engage' ) ) . '</strong></div>';
                        //exit();
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-text-{$this->parent->args['opt_name']}", esc_html__( 'All Defaults Restored!', 'engage' ) ) . '</strong></div>';
                         ...
                         * @param string  translated "settings imported" text
                         */
                        echo '<div class="saved_notice admin-notice notice-yellow"><strong>' . apply_filters( "redux-defaults-section-text-{$this->parent->args['opt_name']}", esc_html__( 'Section Defaults Restored!', 'engage' ) ) . '</strong></div>';
                         ...
                         */
                        echo '<div class="saved_notice admin-notice notice-green">' . apply_filters( "redux-saved-text-{$this->parent->args['opt_name']}", '<strong>'. esc_html__( 'Settings Saved!', 'engage' ) ).'</strong>' . '</div>';
                    }
                 ...
                 */
                echo '<div class="redux-save-warn notice-yellow"><strong>' . apply_filters( "redux-changed-text-{$this->parent->args['opt_name']}", esc_html__( 'Settings have changed, you should save them!', 'engage' ) ) . '</strong></div>';
                ...
                echo '<div class="redux-field-errors notice-red"><strong><span></span> ' . esc_html__( 'error(s) were found!', 'engage' ) . '</strong></div>';
                ...
                echo '<div class="redux-field-warnings notice-yellow"><strong><span></span> ' . esc_html__( 'warning(s) were found!', 'engage' ) . '</strong></div>';
                    ...
                    if ( $developer_theme_file ) {
                        ...
                        if ( $core_version && $developer_version && version_compare( $developer_version, $core_version, '<' ) ) {
                            ...
                            <div id="message" class="error redux-message">
                                <p><?php esc_html_e( '<strong>Your panel has bundled outdated copies of Redux Framework template files</strong> &#8211; if you encounter functionality issues this could be the reason. Ensure you update or remove them.', 'engage' ); ?></p>
                            </div>
                            <?php
                            return;
                        }
