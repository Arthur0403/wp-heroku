...
class OneClickDemoImport {
	...
	protected function __construct() {
        ...
        // Veented Demo Page action
        add_action( 'wp_ajax_ocdi_import_demo_page', array( $this, 'import_demo_page_ajax_callback' ) );
	...
	public function create_plugin_page() {
        ...
        $this->plugin_pages_page = add_submenu_page(
			$plugin_page_setup['parent_slug'],
			__( 'Demo Pages', 'engage' ),
			__( 'Demo Pages', 'engage' ),
			$plugin_page_setup['capability'],
            'engage-demo-pages',
            apply_filters( 'pt-ocdi/plugin_demo_pages_page_display_callback_function', array( $this, 'display_demo_pages_page' ) )
		);
    ...
    /**
	 * Veented: Demo Pages display page. 
	 * Output (HTML) is in another file.
	 */
	public function display_demo_pages_page() {
		require_once PT_OCDI_PATH . 'views/demo-pages.php';
	}
	...
	public function admin_enqueue_scripts( $hook ) {
		...
		if ( $this->plugin_page === $hook || $this->plugin_pages_page === $hook  ) {
			...
			wp_localize_script( 'ocdi-main-js', 'ocdi',
				array(
                    ...
                    'import_pages'     => $this->import_pages,
					...
					'texts'            => array(
						...
						'selected_import_title' => esc_html__( 'Selected demo import:', 'engage-core' ),
						'install_now' => esc_html__( 'Install now', 'engage-core' ),
						'requires_plugins' => esc_html__( 'This demo requires plugins', 'engage-core' ),
						'choose_data' => esc_html__( 'Choose data to be imported', 'engage-core' ),
						'plugins_other_tab' => esc_html__( 'If you just activated required plugins in a separate browser tab then you can proceed with the import. Or just refresh this page to check plugins again.', 'engage-core' )
					),
	...
	public function import_demo_data_ajax_callback() {
		...
		$vntd_content_type = $_POST[ 'vntd_content_type' ];
		$vntd_import_content = true;
		$vntd_import_redux = true;
		$vntd_import_widgets = true;
		
		if ( $vntd_content_type == 'widgets' ) {
			$vntd_import_redux = false;
			$vntd_import_content = false;
		} elseif ( $vntd_content_type == 'options' ) {
			$vntd_import_widgets = false;
			$vntd_import_content = false;
		} elseif ( $vntd_content_type == 'content' ) {
			$vntd_import_widgets = false;
			$vntd_import_redux = false;
		}
		...
		if ( $vntd_import_redux == false ) {
			$this->selected_import_files['redux'] = null;
		}
		
		// Import Widgets?
		
		if ( $vntd_import_widgets == false ) {
			$this->selected_import_files['widgets'] = null;
		}
		
		// Import Content?
		
		if ( $vntd_import_content == false ) {
			$this->selected_import_files['content'] = null;
		}
    ...
    public function import_demo_page_ajax_callback() {
        
        //wp_send_json( esc_html__( 'HELLO THERE', 'pt-ocdi' ) );
        ...
        $this->import_type = 'page';
        //wp_send_json( 'Selected index: ' . $this->selected_index );
		...
		if ( ! $use_existing_importer_data ) {
			...
			// Get selected file index or set it to 0.
			$this->selected_index = empty( $_POST['selected'] ) ? 0 : esc_attr( $_POST['selected'] );
            
            // Veented Import XML file
            
            $vntd_new_title = $_POST[ 'vntd_new_title' ];
            
            // TO-DO CHECK IF FILE IS PROPER?
            
            //if ( ! empty( $this->import_files[ $this->selected_index ] ) ) { // Check if demo page exists
                
                //$this->selected_import_files = Helpers::download_import_page_file( $this->import_pages[ $this->selected_index ] ); // Inserting full array of the demo page
                
                $this->selected_import_files = Helpers::download_import_page_file( $this->selected_index );
                
                // Got the file path now basing on the selected index in the array.
                ...
                $log_added = Helpers::append_to_file(
                    sprintf(
                        esc_html__( 'The import files for: %s were successfully downloaded!', 'pt-ocdi' ),
                        $this->import_files[ $this->selected_index ]['import_file_name']
                    ) . Helpers::import_file_info( $this->selected_import_files ),
                    $this->log_file_path,
                    esc_html__( 'Downloaded files' , 'pt-ocdi' )
	...
	private function final_response() {
		...
		if ( empty( $this->frontend_error_messages ) ) {
            ...
            $response['pages_url'] = admin_url( 'edit.php?post_type=page' );

            $response['message'] .= sprintf(
                esc_html__( '%1$s%3$sThat\'s it, all done!%4$s%2$sThe demo import has finished. You may now visit your website and see results.%5$s', 'engage' ),
                '<div class="notice  notice-success engage-import-success"><p>',
                '<br>',
                '<strong>',
                '</strong>',
                '</p><a class="button button-primary engage-button-primary" href="' . home_url() . '">' . esc_html__( 'View Website', 'engage' ) . '</a></div>'
            );
	...
	public function setup_plugin_with_filter_data() {
        ...
        $this->import_pages = Helpers::validate_import_file_info( apply_filters( 'pt-ocdi/import_pages', array() ) );
