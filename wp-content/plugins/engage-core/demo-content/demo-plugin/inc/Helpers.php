...
class Helpers {
    ...
    // Veented: Download Demo Page
    
    public static function download_import_page_file( $import_file_info ) {
        ...
        $demo_file_info = explode( '_', $import_file_info );
        
        $import_file_url = ENGAGE_DEMO_PAGES_PATH . $demo_file_info[0] . '/' . $demo_file_info[1] . '.xml';
        
        //wp_send_json( 'Import file URL: ' . $import_file_url );
        
		if ( empty( $import_file_info['import_file_url'] ) ) {
			if ( file_exists( $import_file_url ) ) {
				$downloaded_files['content'] = $import_file_url;
			} else {
                wp_send_json( 'File doesn\'t exist: ' . $import_file_url );
            }
		} else {
...
//				$downloaded_files['content'] = $import_file_info['local_import_file'];
//			} else {
//                wp_send_json( 'File doesn\'t exist: ' . $import_file_info['local_import_file'] );
//            }
//		} else {
//            
//			// Set the filename string for content import file.
